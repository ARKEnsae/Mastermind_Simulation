...
\item $\{f(\cdot;v),\,v\in\mathcal{V}\}$ famille discrète de probabilités sur $\mathcal{X}$, paramétrée par $v$. 
\end{itemize}
...
\begin{itemize}
\item 
\textcolor{processblue}{$\hat{v}_{0} = \left(\frac{1}{m}\right)_{i=1..n,j=1..m}$} (probabilités uniformes pour chaque couleur)
\item \textcolor{processblue}{$N = C\times m \times n $} ($C=5$ par défaut)
\item \textcolor{processblue}{$\rho = 0,1$} (maximisation réalisée sur les 10 \% meilleurs échantillons)
...
\textcolor{processblue}{\begin{equation}
...
\item Arrêt : si pour un certain $t\geq d$ (\textcolor{processblue}{$d = 5$}), on a : 
...
couleur ne peut apparaître qu’une seule fois, donc \(m \leq n\)).
...
\begin{frame}{Adaptation de l’algorithme précédent (1/2)}
\protect\hypertarget{adaptation-de-lalgorithme-pruxe9cuxe9dent-12}{}

Il y a \textbf{deux principaux changements} par rapport à l’algorithme
précédent :
...
\item Les échantillons sont désormais générés grâce à une loi \og sans remise \fg{}.
...
\begin{frame}{Adaptation de l’algorithme précédent (2/2)}
\protect\hypertarget{adaptation-de-lalgorithme-pruxe9cuxe9dent-22}{}
...
\textcolor{processblue}{1_{\{X_{i}\text{ permutation}\}}}
}{
...
\begin{itemize}
\item 
\textcolor{processblue}{Tirage aléatoire de $x^*_0$ et  $\lambda_0=1$.}
\item $N = C\times \textcolor{processblue}{(n+1)} $ ($C=5$ par défaut)
\item $\rho = 0,1$ (maximisation réalisée sur les 10 \% meilleurs échantillons)

\end{itemize}
 
\item $X_{1},\dots,X_{N}$ générés avec la \textcolor{processblue}{loi $\pi_{\lambda_t,x^*_t}$} (Metropolis-Hastings), calcul de 
$\hat{\gamma}_{t}=S_{\lceil(1-\rho)N\rceil}$

\item \textcolor{processblue}{Trouver $\tilde x_{t+1}$. Si $S(\tilde x_{t+1})\geq S(x^*_t)$ alors $x^*_{t+1} = \tilde x_{t+1}$, sinon $x^*_{t+1}=x^*_{t}$. On fixe $\lambda_{t+1}=1$}

\item \emph{[FACULTATIF] \textcolor{processblue}{Pas de smoothed updating}}

\item \textcolor{processblue}{Arrêt : si pour un certain $t$, $S(x^*_{t})=1$ alors on arrête l'algorithme.}
...
\item Itération : 
\begin{itemize}
\item On permute au hasard deux éléments de $x_t$ et on note $x'$ la nouvelle permutation.
...
Pour \(m>n\), nous raisonnons de la même façon en modifiant deux étapes
:
...
\item Dans le calcul de la probabilité d'acceptation nous ne calculons la distance de Hamming que sur les $n$ premières coordonnées $x'$ et $x_t$.
\end{itemize}
...
\begin{enumerate}
\item décroissance linéaire à chaque itération
\item constance
