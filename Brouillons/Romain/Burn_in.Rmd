...
Quand $m$ passe à 6, le *burn-in* semble légèrement augmenter par rapport au cas précédent, avec l'apparition d'un plateau après 1000 pour la première coordonnée, nous pourrions donc plutôt envisager un *burn-in* de 1 500 dans ce cas.
...
Pour $m = 20$, on remarque sur la trace de la première coordonnée un long moment entre 4 000 et 5 000 où l'on ne semble plus atteindre des valeurs au-dessus de 15, on peut donc conseiller un *burn-in* de 5 000 ici.
...
L'augmentation de $n$ par rapport au cas précédent ne semble pas avoir modifié l'allure des traces outre mesure, le *burn-in* de 5 000 reste donc toujours acceptable.
...
Une nouvelle augmentation de $m$ à 50 allonge de nouveau le *burn-in*. En effet, on remarque que l'ensemble est valeurs est parcouru à partir de l'itération 7 500 environ, on peut donc prendre cette valeur comme burn-in.
...
L'augmentation de $n$ semble cette fois-ci avoir un peu modifié les traces, notamment pour la 1ère coordonnée, on a plutôt envie d'estimer le *burn-in* aux alentours de 10 000.
