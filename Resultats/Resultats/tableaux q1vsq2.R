...
install.packages(c("kableExtra", "plot3D", "shinyjs", "shinyWidgets",
                   "shiny", "mvtnorm", "portes", "clue"))
set.seed(1)
m = 33
n = 33
...
all_models <- apply(all_models_names,1, function(x){
  ...
  lapply(liste_seeds, function(seed){
    set.seed(seed)
    print(sprintf("n = %i - m = %i - seed = %i", n, m, seed))
...
saveRDS(stats,file = "statsq1vsq2.RDS")
saveRDS(all_models,file = "q1vs2.RDS")
