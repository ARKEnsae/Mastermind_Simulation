...
Dans l'algorithme de Cross-Entropy nous utilisons $\rho = 0,1$ (à l'étape 3 la maximisation est donc faite sur les 10 \% meilleurs échantillons), $N = C\times\text{nombre de paramètres}$ (avec $C=5$ par défaut) et $d=5$.
...
2. Itération :  
  ...
  a. On permute au hasard deux éléments de $x_t$ et on note $x'$ la nouvelle permutation (on fait donc une transposition de $x'$).
...
1. Il générer la loi cible, il construit itérativement une chaîne de Markov qui converge vers cette loi cible. Les échantillons initiaux peuvent donc suivre une distribution très différente de celle recherché. Il est donc nécessaire de rejetter une partie importante des échantillons initiaux (*burn-in*).

2. Les échantillons proches sont généralement autocorrélés. Pour générer des échantillons indépendants il faut donc rejeter la majorité d'entre eux et ne garder que les $n$^ièmes^ échantillons. Ce $n$ est trouvé en calculant l'autocorrélation entre les échantillons et en prenant l'entier tel que $X_t$ ne soit plus corrélé à $X_(t+n)$. Cette autocorrélation peut-être calculée avec la fonction `portes::LjungBox` (la fonction `acf` ne calculant les autocorrélations que pour les séries univariées).
...
$$
Dans notre cas, le problème est que nous observant une croissance rapide de $\lambda$ à chaque itération de la méthode de Cross-Entropy. Ainsi, si $y$ n'est pas décodé dans les premières itérations, les échantillons $X_i$ générés seront très proches de $x^*$ (voir partie \@ref(sec:mh)) et on ne trouvera pas $y$.
...
## Question 1 {#sec:codeq1}

## Question 2 {#sec:codeq2}
