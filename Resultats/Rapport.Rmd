...
Une application web de démonstration faite également à l'aide de `R` (package `shiny`) est disponible à l'adresse suivante : http://arkensae.shinyapps.io/mastermind. 
...
Le nouvel algorithme appliqué à cet exemple converge cette fois-ci plus tôt, à la 8ème étape, toujours vers la bonne valeur de y. Le temps d'exécution (0,1 seconde) est supérieur à précédemment car la loi de génération des échantillons sans remise est plus chronophage.
...
**Remarque importante** : Dans notre cas, le problème est que nous observons une croissance rapide de $\lambda$ à chaque itération de la méthode de *Cross-Entropy*. Ainsi, si $y$ n'est pas décodé dans les premières itérations (ce qui est souvent le cas pour $m$ et $n$ grands), les échantillons $X_i$ générés seront très proches de $x^*$ (voir partie \@ref(sec:mh)) et on ne trouvera pas $y$. Ce problème vient sûrement du fait que les permutations qui ont les meilleurs scores (sur lesquels est faite la maximisation) sont corrélées entre elles : le $\lambda$ qui représente cette distribution est donc grand. De plus le problème d'optimisation n'avait pas toujours de solution sur $[0,10]$, même pour des petites valeurs de $m$ et $n$ de $m$ et $n$ (voir par exemple partie \@ref(sec:codeq3)).   
Nous avons testé plusieurs approches pour $\lambda$ : en le faisant croître linéaire (l'idée étant que plus on se rapproche de la solution, plus les simulations doivent être proches de $x^*$), constant et maximum de vraisemblance. La solution qui donnait les meilleurs résultats est de prendre $\lambda = 1$.

### Résultats

Les tableaux \@ref(tab:tabq3convmed) à \@ref(tab:tabq3tempsconv) montrent qu'il n'y a pas toujours convergence de $x^*$ vers $y$ en 100 itérations. 

Pour comparer cette algorithme à ceux des parties précédentes, nous avons relancé les algorithmes avec $N=5\times n \times m$ simultations. Les résultats sont dans les tableaux \@ref(tab:tabq3vsq1convmed) à \@ref(tab:tabq3vsq1convmed). Augmenter $N$ permet une convergence plus rapide dans le cas où $n<m$ mais ce n'est pas forcément le cas lorsque $m=n$. En revanche, cela augmente beaucoup le temps de calcul.  
Par rapport aux algorithmes précédents, utiliser une loi spécifique pour les permutations donne de moins résultats. Cette algorithme pourrait
  ...
  while(critere_arret & (iter+1)<= maxIters){
    ...
    if(mle){
      ...
      lambda <- tryCatch(uniroot(gradient, c(0,10))$root, error = function(e){
        print("Pas de solution pour lambda : on fixe lambda = 1")
        1})
...
resultat_mle <- lancer_algorithme_hamming(y = y, n = n, m = m,
                              ...
                              # Pour changer x* même si le score est moin bon :
